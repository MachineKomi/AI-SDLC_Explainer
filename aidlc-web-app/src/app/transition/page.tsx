  ...
  return (
    <main className="min-h-screen p-4 md:p-8">
      <header className="mb-6">
        ...
        <p className="text-slate-400">From Agile/Scrum to AI-DLC</p>
      </header>
            ...
            <div key={role.agileRole} className="card">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="border-l-4 border-blue-500 pl-4">
                  ...
                  <p className="text-sm text-slate-500 mb-2">Agile/Scrum</p>
                  <ul className="text-sm space-y-1">
                    {role.agileResponsibilities.map((resp, idx) => (
                      <li key={idx} className="text-slate-400">• {resp}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="border-l-4 border-green-500 pl-4">
                  ...
                  <p className="text-sm text-slate-500 mb-2">AI-DLC</p>
                  <ul className="text-sm space-y-1">
                ...
                <div>
                  ...
                  <ul className="text-sm space-y-1">
                    {role.skillsToDevelop.map((skill, idx) => (
                      <li key={idx} className="text-slate-400">📚 {skill}</li>
                    ))}
                  </ul>
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-slate-500">{process.agileDuration}</span>
                  </div>
                  <p className="text-sm text-slate-400">{process.agilePurpose}</p>
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-slate-500">{process.aidlcDuration}</span>
                  </div>
                  <p className="text-sm text-slate-400">{process.aidlcPurpose}</p>
                </div>
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                <th className="text-left p-3 text-green-400">AI-DLC Artifact</th>
                <th className="text-left p-3 text-slate-400">Key Differences</th>
              </tr>
            </thead>
            <tbody>
              {ARTIFACT_MAPPINGS.map(artifact => (
                <tr key={artifact.agileArtifact} className="border-b border-slate-800">
                  <td className="p-3">
                    <div className="font-medium">{artifact.agileArtifact}</div>
                    <div className="text-sm text-slate-500">{artifact.agilePurpose}</div>
                  </td>
                  <td className="p-3">
                    <div className="font-medium">{artifact.aidlcArtifact}</div>
                    <div className="text-sm text-slate-500">{artifact.aidlcPurpose}</div>
                  </td>
                  <td className="p-3">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm px-2 py-0.5 bg-slate-700 rounded">{phase.duration}</span>
                  </div>
                  <p className="text-slate-400 mb-4">{phase.focus}</p>
                  
                  <div className="grid md:grid-cols-3 gap-4">
                    <div>
                      <h4 className="text-sm font-semibold text-slate-300 mb-2">Activities:</h4>
                      <ul className="text-sm space-y-1">
                        {phase.activities.map((activity, i) => (
                          <li key={i} className="text-slate-400">• {activity}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      ...
                      <ul className="text-sm space-y-1">
                        {phase.successCriteria.map((criteria, i) => (
                          <li key={i} className="text-slate-400">✓ {criteria}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      ...
                      <ul className="text-sm space-y-1">
                        {phase.risks.map((risk, i) => (
                          <li key={i} className="text-slate-400">⚠️ {risk}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                ...
                {READINESS_CHECKLIST.filter(item => item.category === category).map(item => (
                  ...
                  >
                    <input
                      ...
                      className="mt-1 w-5 h-5 rounded border-slate-600 bg-slate-800 text-accent-success focus:ring-accent-primary"
                    />
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className={`font-medium ${checkedItems.has(item.item) ? 'line-through text-slate-500' : ''}`}>
                          {item.item}
                        </span>
